...
2. **[01-runtime-flow.md](01-runtime-flow.md)** — what happens per turn, end-to-end. Walk a phone-call utterance from the voice provider → gateway → brain → LLM → response. The provider can be **Vapi or Telnyx** now — the gateway sits behind a `VoiceProvider` interface and picks one via the `VOICE_PROVIDER` env (default `vapi`), so most of the per-turn flow is provider-agnostic.
...
4. **[03-prompt-and-conversion.md](03-prompt-and-conversion.md)** — the system prompt that drives the agent's behavior: voice rules, persistent probe, offers ladder, hard-no list, the whole conversion playbook. The prompt also branches on `call_mode` (`INBOUND_PRESALES` vs `OUTBOUND_RECOVERY`) — same agent, different opener and mission depending on who dialed whom.
...
- Vapi or Telnyx specifics — both voice providers are explained as we go
...
- Snippets track the current implementation. The `feat/human-feel-pacing`
  work (live customer/cart context, sentiment-adaptive prompts, thinking fillers,
  voice tuning) is now **merged to main**, as is the multi-provider voice
  abstraction (Vapi + Telnyx behind one interface). Line numbers drift as code
  changes — treat them as a starting point, not gospel.
