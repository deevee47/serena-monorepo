...
4. **Build context** — looks up the current product from the **DB-loaded catalog** (no longer hardcoded — see §4.1), fetches a **cheaper alternative AND a premium alternative in parallel** from Pinecone (both category-filtered), and loads the caller's **live customer profile + real abandoned cart** via `loadCallContext()` (cached per call in Redis `call_ctx:<callId>`, ~30 min TTL) plus a `getRecentTurnSignals()` snapshot — see §4.9. Conversation history comes from the Redis session (authoritative — it holds the agent's actually-streamed text, including interrupted turns), falling back to Vapi's `messages[]` only on the very first turn. The premium alt is the "anchor up" lever — the agent uses it to make the current product feel right-sized rather than expensive.
...
2. **Stream the LLM** via `converse_response_stream(...)` ([llm.py:132](fastapi-brain/app/services/llm.py#L132)):
   ...
   - **Observation-tool loop:** if the model called any of the 5 observation tools (`check_inventory | get_recent_purchases | get_review_summary | get_delivery_eta | get_available_offers`), the brain emits a `{type: 'thinking', tool}` event (so the gateway can fire a TTS filler — see §4.9), runs them server-side via `execute_observation_tool(db, name, args)` ([observations.py](fastapi-brain/app/services/observations.py)) against the live Prisma client (mounted on `app.state.db` in [main.py:28](fastapi-brain/app/main.py#L28)), emits a `{type: 'observation', ...}` event with the result, appends the `tool` message back into the conversation, and re-streams from OpenAI. Bounded by `MAX_TOOL_TURNS = 4` to prevent runaways.
...
| [services/db.service.ts](node-gateway/src/services/db.service.ts) | Prisma writes: `createCallRecord`, `updateCallRecord` (P2025-tolerant — orphan call-end jobs from crashed runs no longer retry forever), `insertCallTurn`, `updateCallTurnAnalytics` (classify-analytics worker), `incrementCustomerCallsCount` + `getToolDispatchSummary` (call-end worker). Also `loadCallContext` (live customer + abandoned-cart hydration) and `getRecentTurnSignals` (sentiment / filler / length-trend snapshot) — see §4.9. |
| [services/thinking-filler.ts](node-gateway/src/services/thinking-filler.ts) | Short TTS fillers ("one sec, checking stock —") streamed while an observation tool runs — see §4.9. EN/HI tables, language detection, suppressed when the LLM already opened with its own disfluency. |
...
**Vestigial legacy columns** — `scoring_config` plus several columns survive from the rules-engine era and are no longer meaningfully written: `CallTurn.stage` / `scoreBefore` / `scoreAfter`, `Call.finalScore` / `stageReached`, and the Redis session's `stage` / `score` fields. The converse pipeline writes them with placeholder values (`stage:'INTRO'`, `score:50`, `0/0`); outcome is derived from `tool_called`, not from any of these. Safe to drop in a future migration.

**Migrations** — seven in [prisma/migrations](prisma/migrations/):
...
- `call_ctx:<callId>` — 30-min TTL cache of the caller's live customer profile + abandoned cart (`loadCallContext` result), so Postgres is hit once per call rather than once per turn.
...
| [smoke-test-classifier.py](scripts/smoke-test-classifier.py), [smoke-test-generate-tactic.py](scripts/smoke-test-generate-tactic.py) | Older smoke tests (the latter is dead — from the pre-converse era). |

### 4.9 Human-feel pacing

A cross-cutting layer (added on the `feat/human-feel-pacing` branch) that makes the agent sound like a person on a real call rather than a templated bot. Five pieces:

**Live customer + cart context.** `loadCallContext(phoneNumber)` ([db.service.ts](node-gateway/src/services/db.service.ts)) hydrates the real customer profile (segment, lifetime value, past orders) and the most-recent **abandoned cart** (items, total, `abandoned_minutes_ago`) from Postgres. The gateway caches the result per call in Redis (`call_ctx:<callId>`, 30-min TTL) so it hits Postgres once per call, not once per turn, and passes it to the brain as `customer_context` / `cart_context`. When a call was triggered without an explicit `product_id`, the gateway also adopts the cart's first product as `currentProductId`.

**Sentiment-adaptive behavior.** `getRecentTurnSignals(callId, 3)` reads the last few USER turns and derives a `RecentUserSignals` snapshot: `sentiments[]` (from the async classify-analytics tags), `filler_density` (hesitation tokens/phrases), `length_trend` (linear-regression slope of utterance lengths — negative means disengaging), and `repeated_objection` (same `objection_type` twice in a row). The brain's `converse_prompt_builder._adaptive_behavior()` turns those into an in-context **ADAPTIVE BEHAVIOR** block — soften and shorten on a negative streak, slow down + ask a diagnostic on high filler density, switch tactic on a repeated objection, open the humor budget on a positive streak. It's inserted between the principles and tool guidance so the model reads it as a situational override, not a hard rule. (The brain has its own [`signals.py`](fastapi-brain/app/services/signals.py) with the same filler/length math; the gateway re-implements it locally to avoid a round-trip.)

**Thinking fillers.** Observation tools cost a Postgres round-trip (~200–700 ms). Before awaiting one, the brain emits a `{type: 'thinking', tool}` SSE event; the gateway's [thinking-filler.ts](node-gateway/src/services/thinking-filler.ts) responds by streaming a short TTS-bound phrase ("one sec, checking stock —" / Hindi "ek second, stock dekh leti hoon —") so the customer never hears dead air. Filler language is sniffed from the caller's actual utterance (Devanagari + Romanized-Hindi tokens), falling back to timezone. It is suppressed when the LLM's own text already opened with a disfluency cue, so the customer never hears two thinking-aloud openers stacked.

**Voice tuning.** Outbound calls ([routes/calls.ts](node-gateway/src/routes/calls.ts)) send Vapi `assistantOverrides` tuned for a sales call: `silenceTimeoutSeconds: 12`, `responseDelaySeconds: 0.4`, `numWordsToInterruptAssistant: 2`, `backchannelingEnabled: true`, and `endCallPhrases` (EN + Hindi). Voice selection is timezone-aware — `VAPI_VOICE_HI` for `Asia/Kolkata` / `Asia/Calcutta`, else `VAPI_VOICE_EN` — and a `custom-llm` model override pointing at `VAPI_CUSTOM_LLM_URL` is added when that env var is set.

**New prompt blocks.** `prompt_sections.py` gained `LANGUAGE_RULES` (mirror the customer's language; emit **Romanized** Hindi only — the TTS engine mispronounces Devanagari) and `DISFLUENCY_AND_HUMOR` (thinking-aloud openers, soft acknowledgments, ≤1 joke/call). `converse_prompt_builder._objective()` now defines the agent as explicitly **female**, with detailed Hindi feminine-verb-form rules. The full per-call composition order: objective → LANGUAGE_RULES → VOICE_RULES → DISFLUENCY_AND_HUMOR → call-opening → principles → ADAPTIVE BEHAVIOR (conditional) → tool guidance → local-time → customer → cart → product/alts → discounts → HARD_CONSTRAINTS.
...
It was replaced with ~600 LOC: a single function-calling LLM call per turn (`converse_response_stream`), 7 tool definitions, an observation-tool loop, and an analytics-only classifier worker. The LLM decides whether to talk, look something up, or fire a side effect. Sales competence lives entirely in the system prompt + tool schemas.
...
- Local hot-reload (no Docker): `docker-compose up -d redis`, then run gateway with `bun --watch src/server.ts` (its `dev` script), brain with `uvicorn app.main:app --reload`, worker with `bun src/workers.ts`. The root `bun run dev` wraps the gateway + brain via `concurrently`.
- `make simulate-call` → end-to-end script against the gateway (5 scripted utterances, no Vapi required).
